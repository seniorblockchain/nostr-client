...
import { NostrNavigationItem, NostrVerticalNavigationAppearance, NostrVerticalNavigationMode, NostrVerticalNavigationPosition } from '@theme/components/navigation/navigation.types';
...
@Component({
    selector       : 'nostr-vertical-navigation',
    templateUrl    : './vertical.component.html',
    styleUrls      : ['./vertical.component.scss'],
    animations     : nostrAnimations,
    encapsulation  : ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs       : 'nostrVerticalNavigation'
})
export class NostrVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: NostrVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: NostrVerticalNavigationMode = 'side';
    @Input() name: string = this._nostrUtilsService.randomId();
    @Input() navigation: NostrNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: NostrVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output() readonly appearanceChanged: EventEmitter<NostrVerticalNavigationAppearance> = new EventEmitter<NostrVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<NostrVerticalNavigationMode> = new EventEmitter<NostrVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<NostrVerticalNavigationPosition> = new EventEmitter<NostrVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<NostrNavigationItem> = new ReplaySubject<NostrNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<NostrNavigationItem> = new ReplaySubject<NostrNavigationItem>(1);
    ...
    private _nostrScrollbarDirectives!: QueryList<NostrScrollbarDirective>;
    private _nostrScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any
    {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'nostr-vertical-navigation-animations-enabled'             : this._animationsEnabled,
            [`nostr-vertical-navigation-appearance-${this.appearance}`]: true,
            'nostr-vertical-navigation-hover'                          : this._hovered,
            'nostr-vertical-navigation-inner'                          : this.inner,
            'nostr-vertical-navigation-mode-over'                      : this.mode === 'over',
            'nostr-vertical-navigation-mode-side'                      : this.mode === 'side',
            'nostr-vertical-navigation-opened'                         : this.opened,
            'nostr-vertical-navigation-position-left'                  : this.position === 'left',
            'nostr-vertical-navigation-position-right'                 : this.position === 'right'
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for nostrScrollbarDirectives
     */
    @ViewChildren(NostrScrollbarDirective)
    set nostrScrollbarDirectives(nostrScrollbarDirectives: QueryList<NostrScrollbarDirective>)
    {
        // Store the directives
        this._nostrScrollbarDirectives = nostrScrollbarDirectives;

        // Return if there are no directives
        if ( nostrScrollbarDirectives.length === 0 )
        {
            return;
        }

        // Unsubscribe the previous subscriptions
        if ( this._nostrScrollbarDirectivesSubscription )
        {
            this._nostrScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._nostrScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() => {
                    ...
                    nostrScrollbarDirectives.forEach((nostrScrollbarDirective) => {
                        nostrScrollbarDirective.update();
                    });
                });
    }
    ...
    ngAfterViewInit(): void
    {
        ...
        setTimeout(() => {
            ...
            if ( !this._navigationContentEl.nativeElement.classList.contains('ps') )
            {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.nostr-vertical-navigation-item-active');
            ...
            else
            {
                ...
                this._nostrScrollbarDirectives.forEach((nostrScrollbarDirective) => {

                    // Skip if not enabled
                    if ( !nostrScrollbarDirective.isEnabled() )
                    {
                        return;
                    }

                    // Scroll to the active element
                    nostrScrollbarDirective.scrollToElement('.nostr-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
     ...
     * @param item
     */
    openAside(item: NostrNavigationItem): void
    {
     ...
     * @param item
     */
    toggleAside(item: NostrNavigationItem): void
    {
        // Toggle
    ...
    private _showOverlay(): void
    {
        ...
        this._overlay.classList.add('nostr-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if ( this.transparentOverlay )
        {
            this._overlay.classList.add('nostr-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void
    {
        ...
        this._asideOverlay.classList.add('nostr-vertical-navigation-aside-overlay');
