    ...
    }) =>
    {
        ...
        addComponents(
            _.fromPairs(_.map(options.themes, (theme, themeName) => [
                ...
                ))), (value, key) => [[`--nostr-${e(key)}`, value], [`--nostr-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
            ]))
        );
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`nostr.customProps.background.${colorScheme}`);
            const foreground = theme(`nostr.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--nostr-${e(key)}`, value], [`--nostr-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--nostr-${e(key)}`, value], [`--nostr-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
            ...
            theme: {
                extend: {
                    ...
                    colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                        [name, `rgba(var(--nostr-${name}-rgb), <alpha-value>)`],
                        [`on-${name}`, `rgba(var(--nostr-on-${name}-rgb), <alpha-value>)`]
                    ])))
                },
                nostr  : {
                    customProps: {
